$("#submit").on('click', function(e) {
	...
	if (email == '' || username == '' || password == '' || repassword == '' || language == '') {
		$('#error-msg').text('Please fill all the fields before submitting!');
			...
			success: (res) => {
				if (res) {
					...
					if (e.status == 500 &&  e.responseJSON.message.includes('constraint')) {
						$('#error-msg').text('Username or Email already exists. Please login to continue.');
						$('#form-error').slideDown();
					}
				}
			},
